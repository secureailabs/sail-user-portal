...
const getting_started: TGetDocumentationSuccess = {
  ...
  secondaryText:
    'Overview and Tutorials on how to use the SAIL Unified Patient Registry platform.',
...
const data_annotation_tool: TGetDocumentationSuccess = {
  ...
  secondaryText:
    'Documentation for the Data Annotation Tool, for packaging and uploading data sets.',
  documentationUrl: 'http://www.secureailabs.com'
};
...
const documentation_data: TGetAllDocumentationSuccess = {
  documentation: [data_annotation_tool, videos, getting_started]
};

const getAllDocumentationAPIDemo = async (): Promise<
  TGetAllDocumentationSuccess['documentation']
> => {
...
const DocumentationContainer: React.FC = () => {
  const { data, isLoading, status, error, refetch } =
    // @ts-ignore
    useQuery<TGetAllDocumentationSuccess['documentation'], AxiosError>(
      ['documentation'],
      getAllDocumentationAPIDemo,
      { refetchOnMount: 'always' }
    );
  return Documentation({
    status: status,
    //@ts-ignore
    getAllDocumentationData: data,
    //@ts-ignore
    error: error
  });
};
