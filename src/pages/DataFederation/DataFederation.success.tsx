...
import type { TDataFederationSuccessProps } from './DataFederation.types';
import { useParams } from 'react-router';
import PatientSummary from 'src/components/PatientSummary';

const DataFederationSuccess: React.FC<TDataFederationSuccessProps> = ({
  getDataFederationData,
}) => {
    ...
    <>
      ...
      <Card primaryText="">
        <div className="unified-registry-card">
          <div className="unified-registry-card__header">
            <img src={getDataFederationData.Image} />
            <div className='unified-registry-card__title-and-org'>
              <p className="unified-registry-card__title">{getDataFederationData.Name}</p>
              <div>
                <p className="unified-registry-card__owner">Owned by {getDataFederationData.owner_org} - {getDataFederationData.owner_name} - {getDataFederationData.owner_email}</p>
              </div>
            </div>
          </div>
      ...
      <Card primaryText='Data Users' secondaryText={usersOpen ? '\u25b2' : '\u25bc'} secondaryTextColor='black' secondaryTextOnClick={() => setUsersOpen(!usersOpen)}>{usersOpen ?
        <div className='unified-registry-section__rows--users'>
          {/* @ts-ignore */}
          {getDataFederationData.members_data?.users.map((elem) => <div className='unified-registry-section__row'><Text fontWeight='500'>{elem.name}</Text><Text className='access-details-text' color='primary'>Dataset Access Details</Text></div>)}
        ...
        <div className="unified-registry-card">
          ...
          <div className='registry-modification-form'>
            <FormFieldsRenderer
              ...
              // @ts-ignore
              register={register}
              fields={{ owner_org: { label: 'Organization Name', placeholder: getDataFederationData.owner_name || 'Organization Name', type: 'text' }, Description: { label: 'Description', placeholder: getDataFederationData.Description || 'Description', 'type': 'textarea' }, dataModelSpec: { label: 'Data Model Specifications', placeholder: 'Enter Url', type: 'text' }, owner_org_url: { label: 'Request Access', placeholder: 'Enter Url', type: 'text' } }} />
          </div>
        </div>
      </Card>
      <Margin size={5} />
